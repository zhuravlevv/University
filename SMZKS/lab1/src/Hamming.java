...
class Hamming {
    public static void main(String args[]) {
        ...
        int codeWordLength = scan.nextInt();
        int codeWord[] = new int[codeWordLength];

        for (int i = 0; i < codeWordLength; i++) {
            System.out.println("Enter bit no. " + (codeWordLength - i) + ":");
            codeWord[codeWordLength - i - 1] = scan.nextInt();
        }

        System.out.println("You entered:");
        for (int i = 0; i < codeWordLength; i++) {
            System.out.print(codeWord[codeWordLength - i - 1]);
        }
        System.out.println();

        int b[] = generateCode(codeWord);
        ...
        System.out.println();
        receive(b, b.length - codeWord.length);
    }
